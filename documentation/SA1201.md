...
> :memo: The behavior of this rule can change based on the configuration of the `kinderOrder` property in **stylecop.json**. See [Configuration.md](Configuration.md#kind-order) for more information.

## Rule description

A violation of this rule occurs when the code elements within a file do not follow a standard ordering scheme.

To comply with this rule, elements at the file root level or within a namespace should be (by default) positioned in the following order:
...
Within a class, struct, or interface, elements should be (by default) positioned in the following order:
