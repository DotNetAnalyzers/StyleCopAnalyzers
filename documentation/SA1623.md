...
`init` accessors introduced in C# 9 are treated with more flexibility than `set` accessors. A property with `get` and `init` accessors is allowed to use either "Gets" or "Gets or initializes".
