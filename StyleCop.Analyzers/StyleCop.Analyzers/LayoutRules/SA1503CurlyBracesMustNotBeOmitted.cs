...
namespace StyleCop.Analyzers.LayoutRules
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>A violation of this rule occurs when the opening and closing curly braces for a statement have been
    /// omitted. In C#, some types of statements may optionally include curly braces. Examples include <c>if</c>,
    ...
    /// </code>
    ///
    /// <para>Although this is legal in C#, StyleCop always requires the curly braces to be present, to increase the
    /// readability and maintainability of the code.</para>
    ///
    /// <para>When the curly braces are omitted, it is possible to introduce an error in the code by inserting an
    ...
    ///
    /// <para>StyleCop always requires the opening and closing curly braces to be present, to prevent these kinds of
    /// errors:</para>
    ///
    /// <code language="csharp">
    /// if (true)
    ...
    internal class SA1503CurlyBracesMustNotBeOmitted : DiagnosticAnalyzer
    {
        /// <summary>
        /// The ID for diagnostics produced by the <see cref="SA1503CurlyBracesMustNotBeOmitted"/> analyzer.
        /// </summary>
        public const string DiagnosticId = "SA1503";
        private const string Title = "Curly braces must not be omitted";
        private const string MessageFormat = "Curly braces must not be omitted";
        private const string Description = "The opening and closing curly braces for a C# statement have been omitted.";
        ...
        private static void HandleIfStatement(SyntaxNodeAnalysisContext context)
        {
            ...
            if (context.SemanticModel.Compilation.Options.SpecificDiagnosticOptions.GetValueOrDefault(SA1520UseCurlyBracesConsistently.DiagnosticId, ReportDiagnostic.Default) != ReportDiagnostic.Suppress)
            {
