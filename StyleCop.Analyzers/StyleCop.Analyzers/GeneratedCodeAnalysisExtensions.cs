...
namespace StyleCop.Analyzers
{
    ...
    internal static class GeneratedCodeAnalysisExtensions
    {
        ...
        {
            return (context.Node.SyntaxTree?.IsGeneratedDocument(context.CancellationToken) ?? false) || (context.Node?.HasGeneratedAttribute() ?? false);
        }
        ...
        /// </summary>
        internal static bool IsGeneratedSymbol(this SymbolAnalysisContext context)
        {
            if (context.Symbol == null) return false;
            foreach (var syntaxReference in context.Symbol.DeclaringSyntaxReferences)
            {
                if (syntaxReference.SyntaxTree.IsGeneratedDocument(context.CancellationToken)) return true;
