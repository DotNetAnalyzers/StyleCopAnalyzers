namespace TestHelper
{
    ...
    public abstract partial class DiagnosticVerifier
    {
        ...
        {
            VerifyDiagnosticResults(await this.GetSortedDiagnosticsAsync(sources, language, analyzers, cancellationToken).ConfigureAwait(false), analyzers, expected);
            ...
            if (expected.Any(x => x.Id.StartsWith("SA") || x.Id.StartsWith("SX")))
            {
                ...
                VerifyDiagnosticResults(await this.GetSortedDiagnosticsAsync(sources.Select(x => " // <auto-generated>\r\n" + x).ToArray(), language, analyzers, cancellationToken).ConfigureAwait(false), analyzers, expectedResults);
            }
        }
